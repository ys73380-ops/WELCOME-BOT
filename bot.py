...
MALE_NAMES = {
    ...
    'vipin','vineet','vinod','vishnu','wasim','waseem','zeeshan','zahid',
...
FEMALE_NAMES = {
    ...
    'geeta','gita','garima','gudiya','heena','hema','honey',
    'ishita','isha','ishani','jyoti','juhi',
    'lata','lakshmi','leena','tanvi','tina','tanisha','twinkle',
    'usha','urvashi','veena','vanita','varsha','vandana','vaishnavi',
...
FEMALE_SURNAMES = {'kaur','devi','bai','ben','bhen'}
...
def normalize_name(name: str) -> str:
    ...
    for char in cleaned:
        ...
        nfkd_ascii = "".join(c for c in nfkd if ord(c) < 128 and not unicodedata.combining(c) and c.isprintable())
        if nfkd_ascii:
...
def detect_gender(name: str) -> str:
    ...
    female_score = 0.0
    for word in words:
        if word in MALE_SURNAMES: male_score += 5
        if word in FEMALE_SURNAMES: female_score += 5
    for word in words:
        if word in MALE_NAMES: male_score += 3
        if word in FEMALE_NAMES: female_score += 3
        if word in UNISEX_NAMES: male_score += 0.5; female_score += 0.5
    for mname in MALE_NAMES:
        if len(mname) >= 4 and mname in name_lower: male_score += 0.5
    for fname in FEMALE_NAMES:
        if len(fname) >= 4 and fname in name_lower: female_score += 0.5
    last_word = words[-1] if words else ""
    full_clean = "".join(words)
    for ending in FEMALE_ENDINGS:
        if last_word.endswith(ending) or full_clean.endswith(ending): female_score += 2
    for ending in MALE_ENDINGS:
        if last_word.endswith(ending) or full_clean.endswith(ending): male_score += 2
    if male_score > female_score + 0.5: return "male"
    elif female_score > male_score + 0.5: return "female"
    else: return "neutral"
...
def load_data() -> dict:
    with _data_lock:
        ...
        try:
            ...
            try: os.rename(DATA_FILE, DATA_FILE + ".bak")
            except OSError: pass
            return {}
...
def save_data(data: dict):
    with _data_lock:
        ...
        try: os.replace(tmp, DATA_FILE)
        except OSError: os.rename(tmp, DATA_FILE)
        ...
        if "welcome_messages" in group or "welcome_videos" in group:
            ...
            for gender in ("male", "female", "neutral"):
                ...
                for v in old_vids.get(gender, []):
                    ...
                    if fid: vids.append({"file_id": fid, "is_gif": False})
                if msg or vids: group[gender] = {"message": msg, "videos": vids}
            changed = True
...
def replace_vars(text, full_name, username, mention, gender):
    return (text.replace("{name}", full_name).replace("{username}", username)
               .replace("{mention}", mention).replace("{gender}", gender))
...
async def get_target_chat_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str | None:
    ...
    if not connected:
        await update.message.reply_text(
            ...
            "📌 Group mein jaao aur /connect chalao,\n"
...
async def send_welcome(context, chat_id, user_id, full_name, username, gender):
    ...
    try:
        if videos:
            ...
            if is_gif:
                await context.bot.send_animation(chat_id=int(chat_id), animation=file_id, caption=caption, parse_mode="HTML")
            else:
                await context.bot.send_video(chat_id=int(chat_id), video=file_id, caption=caption, parse_mode="HTML")
        else:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "⚙️ <b>Configuration:</b>\n"
        "├<code>/setmsg male msg</code> ya <code>/setmsg female msg</code>\n"
        "├<code>/setvideomsg male msg</code> ya <code>/setvideomsg female msg</code>\n"
        "├<code>/addvideomsg male</code> ya <code>/addvideomsg female</code>\n"
        "├<code>/delvideos male</code> ya <code>/delvideos female</code>\n"
        "├<code>/showwelcome</code> — Settings dekho\n"
        "└<code>/resetwelcome</code> — Sab reset karo\n\n"
        "🛠 <b>Tools:</b>\n"
        "├<code>/testgender Rahul Singh</code>\n"
        "└<code>/admincheck</code> — Admin check karo\n\n"
        "📌 <b>Variables:</b> <code>{name}</code> <code>{username}</code> <code>{mention}</code> <code>{gender}</code>\n"
        "📌 <b>Genders:</b> male / female / neutral",
        parse_mode="HTML",
...
async def setmsg_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            "❌ <b>Usage:</b>\n\n"
            "<code>/setmsg male</code> Welcome {mention} bhai! 🔥\n"
            "<code>/setmsg female</code> Welcome {mention} didi! 🌸\n\n"
            "📌 Genders: male / female / neutral\n"
            "📌 Variables: {name} {username} {mention} {gender}",
            parse_mode="HTML",
        )
        return
    gender = context.args[0].lower()
    if gender not in ("male", "female", "neutral"):
        await update.message.reply_text("❌ Gender male, female, ya neutral hona chahiye!")
        return
...
# ════════════════════════════════════════════════════════════
# /setvideomsg — VIDEO + MESSAGE
# ════════════════════════════════════════════════════════════

async def setvideomsg_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = await get_target_chat_id(update, context)
    if not chat_id: return
    if not await is_admin_of_chat(update.effective_user.id, chat_id, context):
        await update.message.reply_text("❌ Sirf admins ye command use kar sakte hain!")
    ...
    if not reply or (not reply.video and not reply.animation):
        await update.message.reply_text(
            "❌ <b>Video+Msg set karne ke liye:</b>\n\n"
            "1️⃣ Kisi video/GIF pe reply karo\n"
            "2️⃣ <code>/setvideomsg male</code> Welcome {mention} bhai! 🔥\n"
            "   <code>/setvideomsg female</code> Welcome {mention} didi! 🌸\n\n"
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ Gender to batao!\n\n<code>/setvideomsg male</code> Welcome msg\n<code>/setvideomsg female</code> Welcome msg",
            parse_mode="HTML",
        )
        return
    gender = context.args[0].lower()
    ...
    confirm = f"✅ {emoji} <b>{gender.upper()} Video+Msg Set!</b>\n\n"
    if msg:
        confirm += f"📝 <b>Message:</b> {msg}\n"
    confirm += f"🎥 <b>{vtype} Added!</b> (Total: {vid_count})\n\n"
    confirm += f"💡 <code>/addvideomsg {gender}</code> — Aur videos add karo\n"
    confirm += f"💡 <code>/delvideos {gender}</code> — Videos hatao"
    await update.message.reply_text(confirm, parse_mode="HTML")


# ════════════════════════════════════════════════════════════
# /addvideomsg — AUR VIDEOS ADD KARO
# ════════════════════════════════════════════════════════════

async def addvideomsg_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = await get_target_chat_id(update, context)
    if not chat_id: return
    if not await is_admin_of_chat(update.effective_user.id, chat_id, context):
        await update.message.reply_text("❌ Sirf admins ye command use kar sakte hain!")
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ <b>Usage:</b> Reply to video/GIF +\n\n"
            "<code>/addvideomsg male</code>\n"
            "<code>/addvideomsg female</code>",
            parse_mode="HTML",
        )
        return
    gender = context.args[0].lower()
    if gender not in ("male", "female", "neutral"):
        await update.message.reply_text("❌ male, female, ya neutral!")
        return
    reply = update.message.reply_to_message
    if not reply or (not reply.video and not reply.animation):
        await update.message.reply_text(
...
async def delvideos_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ <b>Usage:</b>\n\n<code>/delvideos male</code>\n<code>/delvideos female</code>",
            parse_mode="HTML",
        )
        return
    gender = context.args[0].lower()
    ...
    if gender_data.get("message"):
        await update.message.reply_text(
            f"✅ {emoji} <b>{gender.upper()} videos deleted!</b>\n\n🗑 Removed: <b>{old_count}</b> videos\n📝 Message abhi bhi set hai!",
            parse_mode="HTML",
        )
    else:
        await update.message.reply_text(
...
async def showwelcome_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for gender in ("male", "female", "neutral"):
        ...
        if vid_count > 0:
            ...
            resp += f"└ 🎥 No videos ❌\n"
        resp += "\n"
    resp += "💡 <code>/setmsg male msg</code> ya <code>/setmsg female msg</code> — Text change karo\n"
    resp += "💡 <code>/setvideomsg male msg</code> ya <code>/setvideomsg female msg</code> — Video+Msg set karo"
...
async def resetwelcome_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in data:
        ...
        save_data(data)
    await update.message.reply_text(
        "✅ <b>Saari settings reset ho gayi!</b>\n\n🔄 Default messages active hain.\n⚙️ <code>/setmsg</code> ya <code>/setvideomsg</code> se naya setup karo!",
        parse_mode="HTML",
    )
...
async def testgender_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("🔍 <b>Usage:</b> <code>/testgender Rahul Singh</code>", parse_mode="HTML")
        return
    ...
    for w in name_lower.split():
        clean = "".join(c for c in w if c.isalpha())
        if clean: words.append(clean)
    details = []
    for word in words:
        if word in MALE_SURNAMES: details.append(f"🏷 '{word}' → Male Surname (+5)")
        if word in FEMALE_SURNAMES: details.append(f"🏷 '{word}' → Female Surname (+5)")
        if word in MALE_NAMES: details.append(f"👤 '{word}' → Male Name (+3)")
        if word in FEMALE_NAMES: details.append(f"👤 '{word}' → Female Name (+3)")
        if word in UNISEX_NAMES: details.append(f"🔄 '{word}' → Unisex (+0.5)")
    emoji = GENDER_EMOJI.get(gender, "🧑")
    detail_text = "\n".join(f"  {d}" for d in details) if details else "  ℹ️ No specific matches"
    await update.message.reply_text(
        f"🔍 <b>═══ GENDER TEST ═══</b>\n\n"
        f"📝 <b>Original:</b> {name}\n✏️ <b>Normalized:</b> {normalized}\n"
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if old_status not in ("left", "kicked", "banned"): return
    if new_status not in ("member", "administrator"): return
    new_member = cm.new_chat_member.user
    chat_id = str(update.effective_chat.id)
    if new_member.is_bot: return
    full_name = (f"{new_member.first_name} {new_member.last_name}".strip()
                 if new_member.last_name else new_member.first_name or "New Member")
    ...
    if gender == "neutral":
        ...
        try:
            if neutral_videos:
                ...
                if is_gif:
                    await context.bot.send_animation(chat_id=int(chat_id), animation=file_id, caption=caption, reply_markup=keyboard, parse_mode="HTML")
                else:
                    await context.bot.send_video(chat_id=int(chat_id), video=file_id, caption=caption, reply_markup=keyboard, parse_mode="HTML")
            else:
                await context.bot.send_message(chat_id=int(chat_id), text=f"{neutral_msg}{question}", reply_markup=keyboard, parse_mode="HTML")
        except Exception as e:
...
async def gender_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    full_name = (f"{user.first_name} {user.last_name}".strip()
                 if user.last_name else user.first_name or "New Member")
...
def main():
    ...
    application.add_handler(CommandHandler("setvideomsg", setvideomsg_cmd))
    application.add_handler(CommandHandler("addvideomsg", addvideomsg_cmd))
